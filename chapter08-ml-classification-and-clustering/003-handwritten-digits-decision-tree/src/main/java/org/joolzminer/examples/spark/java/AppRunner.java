...
public class AppRunner {
	...
	public static void main(String[] args) throws IOException {
		...
		try (JavaSparkContext sc = new JavaSparkContext(config)) {
			...
			decisionTreeClassifier.setMaxDepth(20);
			...
			Pipeline pipeline = new Pipeline()
										.setStages(new PipelineStage[] {labelIndexer, featureIndexer, decisionTreeClassifier, labelConverter});
			
			// Train the model
			PipelineModel pipelineModel = pipeline.fit(penTrainingDataFrame);
			
			// Cast to a DecisionTreeClassificationModel to warp back to the book
			DecisionTreeClassificationModel decisionTreeClassificationModel = (DecisionTreeClassificationModel)(pipelineModel.stages()[2]);

			// In the book this is a categorical node, in my example it is continuous
			System.out.println(decisionTreeClassificationModel.rootNode());
			printSeparator();
